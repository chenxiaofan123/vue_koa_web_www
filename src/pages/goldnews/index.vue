		...
		<v-content class="row">
			...
			<li v-for="item in noticeInfoList">
				...
				<div class="media-summary">
					<p class="title">
						<router-link :to="'goldnewsdetails/'+item.id">{{item.title}}</router-link></p>
					<p>发布时间：{{item.date.substr(0,10)}}</p>
				</div>
			</li>
		</ul>
...
<script>
	 export default {
        ...
        methods: {
           ...
           getData(){
           	var that=this;
           	 this.$http.post('info/getHeadlinesList',{start:this.start,size:this.size,searchCount:true}).then(function(res){
           	 	 var result=res.data;
           	 	 if(result.code==='0000'){
           	 	 	that.noticeInfoList=result.data.headlinesList;
