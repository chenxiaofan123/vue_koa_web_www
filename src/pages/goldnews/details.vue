	...
	<div>
		...
		<v-content>
			<div class=" notice-detail">
				...
				<p class="sectitle">发布时间: {{data.date.substr(0,10)}}</p>
...
<script>
	export default {
		...
		created(){
			var that=this;
			this.$http.post('/info/getHeadlinesDetail',{headlinesId:this.id}).then(function(res){
				var result=res.data;
				if(result.code==='0000'){
					that.data=result.data.headlines;
					var contentarr=[];
						var content=result.data.headlines.content;
						 content=content.replace(/(^\s*)|(\s*$)/g,"");
						 var arr= content.replace(/\s{4,}/g,"    ");
						 arr=arr.split("    ");
...
<style lang="less" scoped>
 @import "../../less/config";
.notice-detail { text-align:center; padding:40px 40px 160px 30px;}
.sectitle{
