		...
		<v-content class="row">
			...
			<li v-for="item in noticeInfoList">
				...
				<div class="media-summary">
					<p class="title">
						<router-link :to="'noticedetails/'+item.id">{{item.title}}</router-link></p>
...
<script>
	 export default {
        ...
        methods: {
           ...
           getData(){
           	var that=this;
           	 this.$http.post('info/getNoticeInfoList',{start:this.start,size:this.size,searchCount:true}).then(function(res){
           	 	 var result=res.data;
           	 	 if(result.code==='0000'){
           	 	 	that.noticeInfoList=result.data.noticeInfoList;
